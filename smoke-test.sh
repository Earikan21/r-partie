...
has "the credit is under an issue too"         'Website built by Emmet' $B/issues
...
# The credit is not a setting, so no amount of posting to the settings form can touch it —
# not even trying to blank it directly, and not a full reset to defaults.
curl -s -b $O -o /dev/null -X POST $B/admin/settings -d "footer_credit=&footer_text="
has "the credit survives a hostile settings write" "Website built by Emmet" $B/
curl -s -b $O -o /dev/null -X POST $B/admin/settings/reset
has "the credit survives a full reset to defaults" "Website built by Emmet" $B/
